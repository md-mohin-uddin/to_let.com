          ...
          <h6 class="card-body-title">
            @foreach($order as $row)
                @if ($row->status == 0)
                    ...
                    <a href="{{route('admin.all.order')}}" class="btn btn-sm btn-info" style="float: right;" >All Rent Request</a> @break
                @else
                    All Rent Request List For Property
                    <a href="{{route('admin.new.order')}}" class="btn btn-sm btn-info" style="float: right;" >New Rent Request</a> @break
                @endif
            @endforeach
          </h6>
          <br>
