...
//--------------user Uploaded Property--------------------------
    public function userUploadedProperty(){
        $property=DB::table('user_properties')
            ...
            ->paginate(10);
            // dd($property);
        return view('pages.user_uploaded_property',compact('property'));
    }

//--------------user Uploaded Property View--------------------------
    public function userUploadedPropertyView($id){
        ...
        if($check){
            ...
            $property=DB::table('user_properties')
                ...
                ->select('user_properties.*','cities.city_name','orders.amount','orders.transaction_id')
                ->where('user_properties.id',$id)
                ->first();
                // dd($property);

            $commentOfProperty = DB::table('property_comments')->where('user_property_id',$id)->orderby('created_at','DESC')->get();

            return view('pages.view_user_uploaded_property',compact('property','commentOfProperty'));
        }
        else{
            $property=DB::table('user_properties')
                ->join('cities','user_properties.city_id','cities.id')
...
//----------------Store_User_added_Property----------------------------
    public function storeUserProperty(Request $request){
        ...
        $data['status']=0;
        $data['property_code']= 'TL-'.mt_rand(100000,999999);
