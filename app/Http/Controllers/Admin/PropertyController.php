...
//-------------- back To Upload Property(button) ------------------
public function backToUploadProperty(Request $request, $id){
    DB::table('user_properties')->where('id',$id)->update(['status'=>1]);

    $data=array();
    $data['user_property_id']=$id;
    $data['comment']=$request->comment;
    DB::table('property_comments')->insert($data);
...
//---------------Pendin_Stage-property(button)-------------------
    public function pendingProperty(Request $request, $id){
